...
dataFileName = os.path.join("data", "fall1.csv")
...
# Manual corrections
a_emp[33] = 11500 # When the microbit hit the ground, it did'nt register the full acceleration. Set it manually.
a_emp[19] = 0 # When the microbit hit the ground, it did'nt register the full acceleration. Set it manually.


# Find the value that microbit sends when standing still.
a_still = a_emp[1:17] # Acceleration when still
a_still_mean = np.nanmean(a_still)

# Convert 
a_emp = ((a_emp / a_still_mean)) * 9.81 - 9.81 # Normalize a_emp to m/s^2
